...
static int migrate_to_node2(struct mm_struct *mm, int source, int dest,
		int flags, int mode)
{
	nodemask_t nmask;
	LIST_HEAD(pagelist);
...
static int access_mm(struct mm_struct *mm, int nid, struct colormask *color_mask, int mode)
{
	nodemask_t nmask;
	LIST_HEAD(pagelist);
	int err = 0;
...
int do_migrate_pages2(struct mm_struct *mm, const nodemask_t *from,
		const nodemask_t *to, int flags, int mode)
{
	int busy = 0;
	int err;
	nodemask_t tmp;
	...
	while (!nodes_empty(tmp)) {
		...
		node_clear(source, tmp);
		err = migrate_to_node2(mm, source, dest, flags, mode);
		if (err > 0)
			busy += err;
		if (err < 0)
			break;
...
static int kernel_migrate_pages2(pid_t pid, unsigned long maxnode,
		const unsigned long __user *old_nodes,
		const unsigned long __user *new_nodes,
		int mode)
{
	...
	err = do_migrate_pages2(mm, old, new,
		capable(CAP_SYS_NICE) ? MPOL_MF_MOVE_ALL : MPOL_MF_MOVE, mode);
...
static int kernel_access_pages2(pid_t pid, int nid,
	unsigned int len, unsigned long __user *user_mask_ptr,
	int mode)
{
	struct colormask mask;
	struct mm_struct *mm = NULL;
	struct task_struct *task;
	int err;

	/* parse color mask */
	err = get_user_color_mask(user_mask_ptr, len, &mask);
	if (err != 0) {
		printk("access_pages2: failed to parse colormask\n");
		goto out;
	}
	...
	err = access_mm(mm, nid, &mask, mode);
...
SYSCALL_DEFINE5(access_pages2, pid_t, pid, int, nid,
	...
	int, mode)
{
	return kernel_access_pages2(pid, nid, len, user_mask_ptr, mode);
}
